...
/* TODO(mmentovai): getValueName returns WebCore::String, a C++ type, prior
 * to upstream svn r30393, which changes it to use a char*.  Include
 * PlatformString.h here until we sync to get the r30393 change.  Note that
 * in the project file, CSSValueKeywords.c has been changed to be a C++ file
 * temporarily to account for the C++ type being used.  When we get the
 * r30393 change, we should be able to revert to compiling that file as
 * straight C, as its extension would indicate. */
#include \"config.h\"
